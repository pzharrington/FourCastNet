...
#      logging.info("time for 1 itr = {}".format(t1))
    ...
    itr_t1 = time.time() - itr_t0
    logging.info("time for 1 itrs = {}".format(itr_t1/100))
    try:
  ...
  def validate_one_epoch(self):
    ...
    if self.params.normalization == 'minmax':
        mult = torch.as_tensor(np.load(self.params.max_path)[0, self.params.out_channels, 0, 0] - np.load(self.params.min_path)[0, self.params.out_channels, 0, 0]).to(self.device)
